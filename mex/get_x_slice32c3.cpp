...
const int CC = 1;    // #channels per plane
const int C  = 3*CC; // #channels in total (for the 3 perpendicular planes)
const int CHANNNEL_OFFSET_TMPL[CC] = {0}; // channel position (offset) template
}

//  X = get_x_slice32c3(img, ind);
//  img: [a,b,c]. int16. the CT volume
//  ind: [M]. linear index to the image for the locations of sampling points
//  X: [32, 32, 3, M]. single. the slices data batch
