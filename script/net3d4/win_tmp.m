...
dir_root  = rootdir();
dir_mo    = fullfile(dir_root,'mo_zoo','tmp3_net3d4_nh16');
...
h.the_dag.p(5).a = f*randn(1,1,1, NH,NH/2, 'single'); 
h.the_dag.p(6).a = zeros(1,NH/2,'single'); 
% parameter layer V, output (full connection)
h.the_dag.p(7).a = f*randn(4,4,4, NH/2,no, 'single'); 
...
tr_bdg = bdg_mhaDefacto2(...
  names, ni_perMha, bs, ...
  @get_x_cubic12, @get_y_g27s5, @bdg_mhaSampBal);
