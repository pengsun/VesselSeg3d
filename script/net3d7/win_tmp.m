...
dir_root  = rootdir();
dir_mo    = fullfile(dir_root,'mo_zoo','tmp_net3d7_nh16');
...
% conv: padding lower
h.the_dag.tfs{ell}.tfs{1}.pad = [1,0, 1,0, 1,0];
h.the_dag.tfs{ell}.tfs{1}.stride = [1,1,1];
% conv: padding higher
h.the_dag.tfs{ell}.tfs{3}.pad = [0,1, 0,1, 0,1];
h.the_dag.tfs{ell}.tfs{3}.stride = [1,1,1];

function h = init_params(h)
NH = 16; % #hidden units
no = 1; % output size
f = 0.1;
% parameter layer I, conv 1
h.the_dag.p(1).a = f*randn(2,2,2, 1,NH, 'single') ; % kernel
h.the_dag.p(2).a = zeros(1, NH, 'single');          % bias
% parameter layer I, conv 2
h.the_dag.p(3).a = f*randn(2,2,2, NH,NH, 'single'); 
h.the_dag.p(4).a = zeros(1,NH,'single');
% parameter layer II, conv 1
h.the_dag.p(5).a = f*randn(2,2,2, NH,NH, 'single'); 
h.the_dag.p(6).a = zeros(1,NH,'single');
% parameter layer II, conv 2
h.the_dag.p(7).a = f*randn(2,2,2, NH,NH, 'single'); 
h.the_dag.p(8).a = zeros(1,NH,'single');
% parameter layer III, conv (full connection)
h.the_dag.p(9).a = f*randn(3,3,3, NH,256, 'single'); 
h.the_dag.p(10).a = zeros(1,256,'single'); 
% parameter layer VI, output (full connection)
h.the_dag.p(11).a = f*randn(1,1,1, 256,no, 'single'); 
h.the_dag.p(12).a = zeros(1,no,'single'); 
...
st = load( fullfile(dir_root, 'data_cache', 'cubic12_ms.mat') );
