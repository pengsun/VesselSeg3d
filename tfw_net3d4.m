classdef tfw_net3d4 < tfw_i
  %TFW_NET3D4 3D ConvNet for segmentation, v4, like Miao Xu's setting,
  % but with regression target
  %   Taking volume patch as input, outputing the foreground score at the
  %   27 grid points.
  %   Max Pooling always take stride 1
  ...
  methods 
    ...
    % Initialize the DAG net connection
      ...
      % -- layer III, 1x1x1 conv, relu, dropout
      ell = ell + 1;
      tfs{ell}   = tf_conv3d();
      tfs{ell}.i = tfs{ell-1}.o;
      ell = ell + 1;
