classdef tfw_net3d9 < tfw_i
  %TFW_NET3D9 3D ConvNet for segmentation, v9
  %   Taking volume patch as input, outputing the foreground scalar score
  %   The second last layer is full connection
  %   Only Conv+Relu, no pooling. Easier for "full connection"
  ...
  methods 
    ...
    % Initialize the DAG net connection
      ...
      % -- layer I: conv3d, relu, conv3d, relu, pool3d
      ell = ell + 1;
      ...
      % -- layer IV, output (full connection), loss
      ell = ell + 1;
      tfs{ell}   = tf_conv3d();
      tfs{ell}.i = tfs{ell-1}.o;
      ell = ell + 1;
